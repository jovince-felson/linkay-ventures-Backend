...
import { EKYC_STEPS, logger, RESPONSE_CODES, publish, Topics, Keys, decryptId, encryptId, SERVICE_URLS, KYC_ENUM } from "rhoam-shared-utils";
...
import { generateShareToken } from "../utils/sumsub/sharetoken.js";
...
export const FetchUserStatus = async (req, res) => {
    try {
        ...
        const profile = await EkycProfiles.findOne({
            where: { user_id: Number(decrypted_user_id) }
        });
...
export const InitiateSumSub = async (req, res) => {
    try {
        ...
        const externalUserId = `USR_${encryptId(decrypted_user_id)}`;
...
export const HandleWebHook = async (req, res) => {
    try {
        ...
        if (payload.reviewStatus !== "completed") {
            ...
            return res.status(200).json({
                success: true,
                message: "Ignoring non-final KYC state",
            });
        }
        ...
        if (ekycProfile.ekyc_status === 1 && reviewAnswer === "GREEN") {
            console.log(27);
            return res.status(200).json({
                success: true,
            ...
            await publish(Topics.EKYC_EVENTS, [
                {
                    ...
                    value: JSON.stringify({
                        user_id: user_id,
                        ekyc_status: ekycProfile.ekyc_status,
                    }),
                }
            ]);
        }
        ...
        if (stepStatus === "FAILED") {
            ekycProfile.attempts += 1;

            if (ekycProfile.attempts >= 3) {
                ekycProfile.ekyc_status = 3;
            } else {
                ekycProfile.ekyc_status = 2;
            }

            ekycProfile.updated_at = new Date();
            await ekycProfile.save();

            if (ekycProfile.ekyc_status === 3) {
                await publish(Topics.EKYC_EVENTS, [
                    {
                        key: Keys.EKYC_VERIFICATION_COMPLETED,
                        value: JSON.stringify({
...
export const GenerateShareToken = async (req, res) => {
    try {
        const user_id = req.headers['x-user-id'] ? decryptId(req.headers['x-user-id']) : 0;
        ...
        const share_token = await generateShareToken(fetch_profile.applicant_id);

        if (!share_token) {
            return res.status(500).json({
                success: false,
                message: 'Share Token Generation Failed',
                response_code: RESPONSE_CODES.OPERATION_FAILED,
            });
        }
        ...
        logger.error("Generate Share Token Error", exception);
        return res.status(500).json({
            success: false,
            message: 'Something went wrong , Please Try Again Later ..!',
            error: exception.message,

...
export const Register = async (req, res) => {
  try {
    ...
    if (existingUser) {
      logger.info("Registration attempted with existing email", {
    ...
    // Token generated here in auth service — email service must not write
    // to the auth DB. 24h expiry per spec.
    ...
    // Store on user row so VerifyEmail can confirm single-use
    // (column is nulled out after first successful verification)
    user.email_verification_token = verification_token;
    await user.save();

    const verification_link = `${CLIENT_URL}/auth/verifyemail?token=${verification_token}`;

    // Downstream analytics / welcome flow
    ...
    // Email service receives a complete ready-to-embed link —
    // no token logic required on that side
    await publish(Topics.USER_EVENTS, [
      {
        key: Keys.USER_EMAIL_VERIFY,
        value: JSON.stringify({
          user_id: user.id,
          email: user.email,
          firstName,
    ...
    logger.error("Register Error", exception);
...
// const EMAIL_VERIFICATION_SECRET = process.env.EMAIL_VERIFICATION_SECRET;
// const CLIENT_URL = process.env.CLIENT_URL; // e.g. https://app.yourplatform.com
...
//     // Generate the verification token here in auth service —
//     // email service has no business writing to the auth DB.
//     // 24h expiry per spec.
//     const verification_token = jwt.sign(
//       { user_id: user.id },
//       EMAIL_VERIFICATION_SECRET,
...
//     // Store against the user row so VerifyEmail can validate
//     // it's single-use (nulled out after first successful verification).
//     user.email_verification_token = verification_token;
//     await user.save();
...
//     // Email service consumer receives a complete, ready-to-embed link.
//     // It only needs to fetch active SMTP config and send — no token logic needed there.
//     await publish(Topics.USER_EVENTS, [
//       {
//         key: Keys.USER_EMAIL_VERIFY,
//         value: JSON.stringify({
